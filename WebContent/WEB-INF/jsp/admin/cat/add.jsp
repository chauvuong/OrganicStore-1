	...
	<section class="wrapper">
		<div class="row">
			<div class="col-lg-12">
				...
				<ol class="breadcrumb">
					...
					<li><i class="fa fa-laptop"></i><a
						href="${pageContext.request.contextPath }/admin/cat/add">Thêm
							Danh Mục</a></li>
				</ol>
			</div>
		</div>
		<c:choose>
			<c:when test="${param['msg'] eq 'add' }">
				<h4>
					<p style="color: red; font-family: cursive;">Tên Danh Mục Đã
						Tồn Tại</p>
				</h4>
			</c:when>
		</c:choose>
							...
							<form action="${pageContext.request.contextPath }/admin/cat/add"
								...
								<div class="form-group">
									<label for="exampleInputEmail1">Tên Danh Mục</label> <input
										...
										<c:if test="${cat != null}"> value="${cat.name}"</c:if>
										id="exampleInputEmail1" placeholder="Nhập Tên Danh Mục" />
								</div>

								<p>
									<form:errors path="cat.name"
