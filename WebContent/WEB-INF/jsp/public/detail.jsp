						...
						<div class="wrap-main-pic-100 bo-all-1 bocl12 pos-relative">
							<div class="main-frame">
								<div class="wrap-main-pic">
									<div class="main-pic">
										<img style="width: 598px; height: 601.73px;"
...
<%-- 
						<div class="wrap-thumb-100 flex-w flex-sb p-t-30">
							<div class="thumb-100">
								<div class="sub-frame sub-1">
									<div class="wrap-main-pic">
										<div class="main-pic">
											<img style="width: 180px; height: 159px;"
												src="${pageContext.request.contextPath }/files/${objNews.picture}"
												alt="${objNews.name}">
										</div>
									</div>
			...
			<div class="col-md-5 col-lg-6">
				<div class="p-l-70 p-t-35 p-l-0-lg">
					<h4 class="js-name1 txt-l-104 cl3 p-b-16">${objNews.name }</h4>
				 	...
				 	<c:choose>
							<c:when test="${numberComment !=0 }">
								...
								<span class="fs-16 cl11 lh-15 txt-center m-r-15"> <c:forEach
										begin="1" end="${numRating}">
										<i class="fa fa-star m-rl-1"></i>
									</c:forEach> <c:forEach begin="1" end="${5- numRating }">
					...
					<div class="flex-w flex-m p-t-55 p-b-30">
						...
						<button onclick="sanpham(${objNews.id_product})"
					...
					<div class="txt-s-107 p-b-6">
						<span class="cl6"> Giá Tiền: </span> <span class="cl9">
							$${objNews.price } </span>
					</div>

					<div class="txt-s-107 p-b-6">
						<span class="cl6"> Lượt Xem: </span> <span class="cl9">
							${objNews.count_views } </span>
					</div>

					<div class="txt-s-107 p-b-6">
						<span class="cl6"> Preview: </span> <span class="cl9">
							${objNews.preview } </span>
					</div>
		...
		<div class="tab02 p-t-80">
			...
			<div class="tab-content">
				...
				<div class="tab-pane fade show active" id="description"
					role="tabpanel">
					<div class="p-t-30">
						<p class="txt-s-112 cl9">${objNews.details }</p>
					</div>
				</div>
					...
					<div class="p-t-36">
						...
						<div class="commentlist">
							<c:choose>
									...
									<c:forEach items="${listComment}" var="objComment">
										<div class="flex-w flex-sb-t bo-b-1 bocl15 p-b-37">
											<div class="wrap-pic-w size-w-56">
												<img
													src="${pageContext.request.contextPath }/templates/public/images/avatar-00.gif"
													alt="AVATAR">
											</div>
												...
												<div class="flex-w flex-sb-m p-b-8">
													...
													<span class="fs-16 cl11 lh-15 txt-center m-r-15 p-tb-5">
														<c:forEach begin="1" end="${objComment.rating}">
															<i class="fa fa-star m-rl-1"} ></i>
														</c:forEach> <c:forEach begin="1" end="${5- objComment.rating }">
															<i class="fa fa-star-o m-rl-1"} ></i>
														</c:forEach>
													</span>
												</div>
								...
								<c:otherwise>
									<h5 class="txt-m-102 cl3 p-b-36">Hãy Là Người Bình Luận
										Đầu Tiên</h5>
								</c:otherwise>
							</c:choose>
						...
						<form onsubmit="return checkComment(event)" class="w-full p-t-42">
							...
							<div class="flex-w flex-m p-b-3">
								<span class="txt-s-101 cl6 p-b-5 m-r-10"> Rating </span> <span
									class="wrap-rating fs-16 cl11 pointer"> <i
									class="item-rating pointer fa fa-star m-rl-1"><input
										value="1" class="dis-none" type="number" id="rating"
										name="rating"></i> <i
									class="item-rating pointer fa fa-star m-rl-1"><input
										value="2" class="dis-none" type="number" id="rating"
										name="rating"></i> <i
									class="item-rating pointer fa fa-star m-rl-1"><input
										value="3" class="dis-none" type="number" id="rating"
										name="rating"></i> <i
									class="item-rating pointer fa fa-star m-rl-1"><input
										value="4" class="dis-none" type="number" id="rating"
										name="rating"></i> <i
									class="item-rating pointer fa fa-star m-rl-1"><input
										value="5" class="dis-none" type="number" id="rating"
										name="rating"></i>
								</span>
							</div>
							...
							<div class="row p-t-12">
								<div class="col-sm-6 p-b-30">
									<input
										...
										type="text" id="name" required="required"
										<c:if test="${userInfo != null}"> value="${userInfo.fullname}"</c:if>
										value="" name="name" placeholder="Tên *">
								</div>

								<div class="col-sm-6 p-b-30">
									<input
										class="txt-s-101 cl3 plh1 size-a-25 bo-all-1 bocl11 focus1 p-rl-20"
										type="text" name="mail" required="required"
										<c:if test="${userInfo != null}"> value="${userInfo.email}"</c:if>
										value="" id="mail" placeholder="Email *">
								</div>
		...
		<div class="wrap-slick9">
			...
			<div class="slick9">
				...
				<c:forEach var="objNews" items="${listNewsTLQ }">
					<c:set var="urlDetail"
						value="${pageContext.request.contextPath}/san-pham/${slugUtil.createSlug(objNews.name)}-${objNews.id_product}.html"></c:set>
